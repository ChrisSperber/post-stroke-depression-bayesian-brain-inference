...
This analysis employs additional control for the variable lesion volume.
...
COVARIATE_TAG = "lesionvolume"
LESION_VOLUME_LOG = "Lesion_volume_log"
...
data[LESION_VOLUME_LOG] = np.log(data[Cols.LESION_VOLUME])

# define covariates
covariates_arr = data[[LESION_VOLUME_LOG]].to_numpy()
...
reference_nifti = load_nifti(
    reference_lesion_path  # pyright: ignore[reportArgumentType]
)
...
if SAMPLE_MODE == SampleSelectionMode.STROKE:
    output_dir = (
        BLDI_OUTPUT_DIR_PARENT
        / COVARIATE_SENSITIVITY_ANALYSIS_SUBDIR
        / f"{OUTPUT_DIR_BASE}_{COVARIATE_TAG}_{timestamp}"
    )
elif SAMPLE_MODE == SampleSelectionMode.STROKE_TRAUMA:
    output_dir = (
        BLDI_OUTPUT_DIR_PARENT
